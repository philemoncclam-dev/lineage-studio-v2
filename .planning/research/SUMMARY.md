...
### Research Flags
- **RESOLVED post-synthesis (orchestrator, verified against `backend/app/purview/writer.py`):** the write-path backend behavior this research pass listed as unconfirmed is now confirmed, and it is *better* than assumed. `WriteSession` is a hard chokepoint through which all three write paths (lineage push, data products, definitions) must pass. Dry-run is the **default**, and critically it is the *same code path* -- the payload is built identically and only transmission is gated, so the preview provably cannot drift from the real write. `WriteOp.describes` is a human-readable statement of intent whose docstring says it exists to be "surfaced in the UI preview." `run()` deliberately does not abort on failure, so partial batches report precisely which operations failed (`writer.py:91-93`). `apply=True` is honoured only when `PURVIEW_ALLOW_WRITE` is also set, and asking to apply without it downgrades to dry-run rather than erroring -- so one endpoint can serve both the preview and the confirm button. `WriteResult.to_dict()` already exposes `dry_run`, `ok`, per-op `operations`, `responses`, and `errors` over the wire. **Consequence: the Purview-push phase is UI-only.** Pitfalls 1/2/4 (qualified-name overwrite, partial-batch failure, false "it landed" confirmation) are structurally mitigated server-side already; the remaining genuine design work is provenance/confidence display and diff-against-existing, not safety plumbing. Do not schedule a research sub-phase for this.
- **Needs deeper research:** knowledge-graph canvas phase (Cosmograph license resolution, realistic Fabric-tenant node-count scale).
...
### Gaps to Address
- ~~Backend write-path behavior (dry-run mode, per-entity results) is unverified against actual code~~ -- **CLOSED.** Verified directly against `backend/app/purview/writer.py`; see the RESOLVED entry under Research Flags. Dry-run-by-default, single non-drifting code path, per-op error reporting, and a double gate (`apply` + `PURVIEW_ALLOW_WRITE`) are all already implemented. This materially shrinks and de-risks the Purview-push phase.

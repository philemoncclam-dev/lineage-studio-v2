...
//
// Configured, it is ONE BUTTON on the orbits — no card, no prose. There is a
// single thing to do here and the button says it. Unconfigured, it becomes a
// card of setup steps, because that screen has something to teach and no button
// to press instead.
//
...
export function LoginPage() {
  ...
  return (
    <main className="lg-page" aria-labelledby="lg-title">
      ...
      {/* The page still needs a name: `aria-labelledby` points here, and a
          landmark announced with one button and no heading tells a screen
          reader user nothing about where they have landed. Hidden rather than
          deleted — the orbits and the button are the design, and this is for
          the readers who cannot see either. */}
      <h1 className="lg-visually-hidden" id="lg-title">
        Lineage Studio
      </h1>
        ...
        <section className="lg-card">
          ...
          <ol className="lg-steps">
            ...
            {/* There is no "Fabric" entry under API permissions — Fabric's
                ...
                to Azure Storage, which OneLake needs separately). Both are easy
                half-hours to lose, so the steps name the exact tiles.
            ...
            <li>
              ...
              <code>DataPipeline.ReadWrite.All</code>. The last two are Fabric's
              price for reading notebook and pipeline source — it has no
              read-only scope for that — and this app never writes.
            </li>
            <li>
          ...
          {allowSkip ? (
            <>
              ...
              <p className="lg-fine">
                Development only. Without sign-in the app reads Fabric using the
                backend's shared service principal, so the workspaces you see
                are <strong>whichever that account can reach</strong> — not
                yours.
              </p>
            </>
          ) : (
            /* Deployed with no app registration. There is deliberately no way
               ...
               would hand the backend's service principal — a real credential on
               a real tenant — to anyone holding the URL. */
            <p className="lg-fine">

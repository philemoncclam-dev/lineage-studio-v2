// Parse a pasted/uploaded schema table (CSV/TSV) and turn it into
// Object > Group(table) > Attribute(column) nodes under a chosen Layer.
import type { LineageNode, Model, NodeType } from "../types";

const newId = () =>
  (crypto.randomUUID?.() ?? Math.random().toString(36).slice(2)).replace(/-/g, "");

/**
 * RFC-4180-compliant CSV line parser. Splits on commas that are outside of
 * double-quoted fields, so values like DECIMAL(10,2) are kept intact.
 * Surrounding quotes are stripped from quoted fields; escaped quotes ("") are
 * unescaped to a single quote character.
 */
...
// i.e. an ordinal like `Customers,CustomerID,1,INTEGER`. Used to auto-detect
// headerless pastes so the user can go straight into mapping.
...
export function parseSchema(text: string): ParsedSchema {
  ...
  if (lines.length === 0)
    return { headers: [], rows: [], delimiter: "comma", hasHeader: false };
  const delimiter = lines[0].includes("\t") ? "tab" : "comma";
  const split = delimiter === "tab"
    ? (l: string) => l.split("\t").map((c) => c.trim())
    : parseCSVLine;
  const cells = lines.map(split);

  // If the first line already looks like data, there is no header row and we
  // fall back to fixed column positions: Table, Column, Ordinal, DataType.
  const hasHeader = !looksLikeDataRow(cells[0]);
  if (!hasHeader) {
    const width = Math.max(4, ...cells.map((c) => c.length));
    const headers = Array.from({ length: width }, (_, i) =>
      ["Table", "Column", "Ordinal", "DataType"][i] ?? `Column ${i + 1}`
    );
...
// Group parsed rows by table name (first-seen order), sorted by ordinal.
...
/**
 * Builds the NEW nodes for the grouped schema. Each table becomes a Group card
 * placed directly under its assigned Layer (no Object band) with its columns as
 * ordinal-sorted Attributes. Existing Layer/Group/Attribute nodes (matched by
 * name + parent) are reused so re-importing doesn't duplicate.
 */
export function buildImportNodes(
  ...
  const make = (type: NodeType, name: string, parentId: string | null): LineageNode => {
    const n: LineageNode = {
      id: newId(),
      type,
      name,
